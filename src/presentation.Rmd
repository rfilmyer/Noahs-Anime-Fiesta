...
```{r}
g <- ratings %>% ggplot() # Common data throughout all plots

g + geom_bar(aes(x = factor(rating))) # Bar plot

# Create a fake grouping variable, for a violin plot of 1 dim 
g + geom_violin(aes(x = factor(0), y = rating), trim = FALSE) + 
...
# School days is a notoriously controversial anime. Is the variance of users' ratings of this anime higher
# than it is for most anime?
```{r}
...
g + geom_violin(aes(x = factor(0), y = as.factor(rating)), trim = FALSE) + 
  xlab("") + scale_x_discrete(breaks = NULL)
```
# Yeah, looks like the 
